...
#include "SysToolLib/SysToolConfig.h"
...
DCI::Bool SimModelComp::Configure()
{
	...
	try
	{
		...
		if (GetOutputPorts()->GetCount() != 2)
			throw "Component must contain exactly two output ports";
...
void SimModelComp::LoadConfigurationFromParameterTable()
{
	...
	if (!m_XMLSchemaCache->SchemaInitialized())
	{
		...
		sVec = *((DCI::StringVector*)hVar->GetValues());
		if (sVec.Len() != 1)
	...
	if(hVar)
	{
		lVec = *((DCI::IntVector*) hVar->GetValues());
		if (lVec.Len() > 0)
		{
			if (lVec[0] == 1)
	...
	if(hVar)
	{
		dVec = *((DCI::DoubleVector*) hVar->GetValues());
		if (dVec.Len() > 0)
		{
...
void SimModelComp::FillOutputTables()
{
	...
	hVar = hTabTimes->GetColumns()->AddNew("Time", dummyVar);
													 ...
													 const std::string Name)
{
	...
	DCI::IAttributeHandle dummyAttr;
	DCI::IVariableHandle dummyVar;

	hVar = hTab->GetColumns()->AddNew(Key.c_str(), dummyVar);
	if(!hVar)
	...
	hAttr = hFieldDef->GetAttributes()->AddNew(conAttrID, dummyAttr);
	hAttr->SetName(conAttrID);
	hAttr->SetStringValue(XMLHelper::ToString(id).c_str());

	hAttr = hFieldDef->GetAttributes()->AddNew(conAttrPath, dummyAttr);
	hAttr->SetName(conAttrPath);
	hAttr->SetStringValue(path.c_str());

	hAttr = hFieldDef->GetAttributes()->AddNew(conAttrType, dummyAttr);
	hAttr->SetName(conAttrType);
	hAttr->SetStringValue(type.c_str());

	hAttr = hFieldDef->GetAttributes()->AddNew(conAttrTimeColumnIndex, dummyAttr);
...
void SimModelComp::FillObserversInputTable()
{
	...
	for(size_t i=0; i<observerInfos.size(); i++)
	{
		...
		hTab->SetValue(i+1, conID, (DCI::Int)observerInfo.GetId());
...
void SimModelComp::UpdateVariableParameters(void)
{
	...
	DCI::IntVector  idVec = *((DCI::IntVector*) hTab->GetColumn(conID)->GetValues());
	DCI::DoubleVector  valueVec = *((DCI::DoubleVector*) hTab->GetColumn(conValue)->GetValues());
	...
	DCI::IntVector idVecPoints      = *((DCI::IntVector*) hTabPoints->GetColumn(conID)->GetValues());
	DCI::DoubleVector XPoints       = *((DCI::DoubleVector*) hTabPoints->GetColumn(conTime)->GetValues());
	DCI::DoubleVector YPoints       = *((DCI::DoubleVector*) hTabPoints->GetColumn(conValue)->GetValues());
	DCI::ByteVector   RestartSolver = *((DCI::ByteVector*) hTabPoints->GetColumn(conRestartSolver)->GetValues());
...
void SimModelComp::UpdateVariableSpeciesProperties(void)
{
	...
	DCI::IntVector  idVec = *((DCI::IntVector*) hTab->GetColumn(conID)->GetValues());
	DCI::DoubleVector initValueVec   = *((DCI::DoubleVector*) hTab->GetColumn(conInitialValue)->GetValues());
	DCI::DoubleVector scaleFactorVec = *((DCI::DoubleVector*) hTab->GetColumn(conScaleFactor)->GetValues());
										   ...
										   QuantitiesSelectionMode selectionMode)
{
	...
	for(size_t i=0; i<simParams.size(); i++)
	{
		...
		hTab->SetValue(LastRowIdx, conID, (DCI::Int)parameterInfo.GetId());
										   ...
										   QuantitiesSelectionMode selectionMode)
{
	...
	for(i=0; i<tableParamInfos.size(); i++)
	{
		...
		for(size_t pointIdx=0; pointIdx<valuePoints.size(); pointIdx++)
		{
			...
			hTab->SetValue(LastRowIdx, conID, (DCI::Int) tableParameterInfo.GetId());
										 ...
										 QuantitiesSelectionMode selectionMode)
{
	...
	for(size_t i=0; i<simSpecies.size(); i++)
	{
		...
		//---- by now, all species can be varied

		//increment row index
		LastRowIdx++;

		hTab->SetValue(LastRowIdx, conID, (DCI::Int)speciesInfo.GetId());
...
vector<ParameterInfo> SimModelComp::GetVariableParameters(void)
{
	...
	DCI::ByteVector isVariableVec = *((DCI::ByteVector*) hTab->GetColumn(conIsVariable)->GetValues());
	DCI::IntVector  idVec         = *((DCI::IntVector*) hTab->GetColumn(conID)->GetValues());
	DCI::StringVector  pathVec    = *((DCI::StringVector*) hTab->GetColumn(conPath)->GetValues());
...
DCI::String SimModelComp::Invoke(const DCI::String &fncName, const DCI::String &args)
{
	...
	try
	{
		if ((fncName == "SaveSimulationToXml") || (fncName == "SaveSimulationToXmlEncrypted"))
		{
			if(!m_ProcessMetaDataPerformed)
			...
			bool saveEncrypted = fncName == "SaveSimulationToXmlEncrypted";

			//save simulation file
			if(NewPath=="") //Overwrite original files
			{
				HelpMe.SaveStringToFile(SimXMLString, m_SimFileName, saveEncrypted);
			}
			else
			{
				HelpMe.SaveStringToFile(SimXMLString, NewPath, saveEncrypted);
			}
		}
